...
#from mot.modeling.track_head#.EMM.module import make_conv3x3

from .module import make_conv3x3
...
class EMMPredictor(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.MODEL.DLA != None:
            in_channels = cfg.MODEL.DLA.BACKBONE_OUT_CHANNELS
        elif cfg.MODEL.RESNETS != None:
            in_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
        else:
            in_channels = 128

        self.cls_tower = make_conv3x3(cfg, in_channels=in_channels, out_channels=in_channels,
                                      use_gn=True, use_relu=True, kaiming_init=False)
        self.reg_tower = make_conv3x3(cfg, in_channels=in_channels, out_channels=in_channels,
                                      use_gn=True, use_relu=True, kaiming_init=False)
        self.cls = make_conv3x3(cfg, in_channels=in_channels, out_channels=2, kaiming_init=False)
        self.center = make_conv3x3(cfg, in_channels=in_channels, out_channels=1, kaiming_init=False)
        self.reg = make_conv3x3(cfg, in_channels=in_channels, out_channels=4, kaiming_init=False)
